...
import com.example.university_schedule.databinding.FragmentFridayBinding
...
class FridayFragment : Fragment() {
   ...
   val shared = context?.getSharedPreferences("UserId", Context.MODE_PRIVATE)
        ...
        listData = listOf()
        when(editor.toString()){
            "2" -> {takeData(requireContext(),"friday") { data ->
        ...
        binding.saveLesson.setOnClickListener {
            ...
            sendLessonData("friday",requireContext(),selectedItem)
